<script>
	...
	let { data } = $props(); 
	const mapels = $derived(data.mapel); 
...
<div class="p-6">
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-2xl font-bold text-white">Daftar Mata Pelajaran</h1>
		<a
			href="/admin/mapel/create"
	...
	<div class="overflow-x-auto rounded-lg border border-zinc-800 bg-zinc-900">		
        ...
        <form 
            ...
            use:enhance = {() => {
                return async ({ result }) => {
					...
					toast.send(`Mapel ${selectedMapelName} berhasil dihapus!`);
		...
		<table class="w-full text-left text-zinc-300">
			...
			<tbody class="divide-y divide-zinc-800">
				{#each mapels as mapel, i (mapel.id)}
					<tr
						class="transition-colors hover:bg-zinc-600/60 {i % 2 === 0
							? 'bg-zinc-900'
							: 'bg-zinc-950'}"
					>
						<td class="px-6 py-4 font-medium text-white">{mapel.kode_mapel}</td>
						<td class="px-6 py-4">{mapel.nama_mapel}</td>
						<td class="px-6 py-4">
							<a
								href="/admin/mapel/{mapel.id}"
						...
						<td class="flex justify-center gap-4 px-4 py-4">
							<a
								href="/admin/mapel/{mapel.id}/edit"
							...
							<button
								type="button"
								onclick={() => confrimDelete(mapel.id, mapel.nama_mapel)}

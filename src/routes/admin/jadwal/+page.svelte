<script>
	...
	let { data } = $props(); 
	const jadwals = $derived(data.jadwal); 
	...
	function confrimDelete(id, gabunganNama) {
		selectedJadwalId = id;
        selectedJadwalName = gabunganNama;
		showModal = true;
	}
...
<div class="p-6">
	<div class="mb-6 flex items-center justify-between">
		<h1 class="text-2xl font-bold text-white">Daftar Jadwal Pelajaran</h1>
		<a
	...
	<div class="overflow-x-auto rounded-lg border border-zinc-800 bg-zinc-900">		
        ...
        <form 
            ...
            use:enhance = {() => {
                return async ({ result }) => {
					...
					toast.send(`Jadwal ${selectedJadwalName} berhasil dihapus!`);
		...
		<ModalConfirm
			open={showModal}
			title="Hapus Jadwal"
			message="Apakah kamu yakin menghapus {selectedJadwalName}?"
				...
				{#each jadwals as jadwal, i (jadwal.id)}
					...
					>
						...
						<td class="flex justify-center gap-4 px-4 py-4">
							...
							<button
								type="button"
								onclick={() => confrimDelete(jadwal.id, `${jadwal.nama_mapel} - ${jadwal.nama_kelas} (${jadwal.hari})`)}
